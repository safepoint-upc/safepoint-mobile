...
class MapaScreen extends StatefulWidget {
  ...
  const MapaScreen({super.key, this.onToggleFullscreen});
...
class _MapaScreenState extends State<MapaScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final results = await Future.wait([
        ...
        prediccionesService.getPrediccionesMapa(),
      ]);
  ...
  Future<void> _generateHeatmap() async {
    ...
    // Scale maxVal to increase intensity (make it more red) like the web dashboard
    maxVal = maxVal * 0.45;
  ...
  List<Polygon> _buildSectoresPolygons() {
    ...
    return sectores.asMap().entries.map((entry) {
      ...
      try {
        final pred = _predicciones.firstWhere((p) => p.sector == sectorId);
        probPromedio = pred.probabilidad;
      } catch (e) {
        // Ignorar si no hay predicción para el sector
      }

      final hasAlert = _alertas.any((a) => a.sector == 'Sector $sectorId' || a.sector == sectorId);

      Color fillBaseColor;
      Color borderBaseColor;

      if (hasAlert || probPromedio > 0.66) {
          ...
          child: Column(
            ...
            children: [
              ...
              _buildLayerToggle(
                ...
                'Mostrar ubicación exacta de incidentes',
                Icons.pin_drop,
                _mostrarIncidentes,
                (val) {
              ...
              if (_mostrarIncidentes) ...[
                ...
                Padding(
                  ...
                  child: _buildLayerToggle(
                    'Mostrar cantidad por punto',
                    'Agrupa incidentes cercanos',
                    Icons.numbers,
                    _mostrarNumeros,
                    (val) {
...
class _MapControlButton extends StatelessWidget {
  ...
  const _MapControlButton({required this.icon, required this.onPressed, required this.isDark});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      height: 40,
      decoration: BoxDecoration(
        color: (isDark ? AppTheme.surface : Colors.white).withValues(alpha: 0.92),
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        const SizedBox(width: 6),
        Text(label, style: TextStyle(color: isDark ? AppTheme.textSecondary : Colors.black54, fontSize: 9)),
      ],
    );
  }
}
